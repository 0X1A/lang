...
pub trait Visitor<T>: Sized {
    fn visit_expr(&self, expr: &Expr) -> Result<T, LangError> {
        Ok(visit_expr(self, expr)?)
    }
    fn visit_stmt(&self, stmt: &Stmt) -> Result<T, LangError> {
        Ok(visit_stmt(self, stmt)?)
    }

    fn visit_assign(&self, assign: &AssignExpr) -> Result<T, LangError>;
    fn visit_binary(&self, binary: &BinaryExpr) -> Result<T, LangError>;
    fn visit_call(&self, call: &CallExpr) -> Result<T, LangError>;
    fn visit_get(&self, get: &GetExpr) -> Result<T, LangError>;
    fn visit_enum_path(&self, enum_path: &EnumPathExpr) -> Result<T, LangError>;
    fn visit_grouping(&self, grouping: &GroupingExpr) -> Result<T, LangError>;
    fn visit_literal(&self, literal: &LiteralExpr) -> Result<T, LangError>;
    fn visit_logical(&self, logical: &LogicalExpr) -> Result<T, LangError>;
    fn visit_set(&self, set: &SetExpr) -> Result<T, LangError>;
    fn visit_unary(&self, unary: &UnaryExpr) -> Result<T, LangError>;
    fn visit_array(&self, array: &ArrayExpr) -> Result<T, LangError>;
    fn visit_index(&self, index: &IndexExpr) -> Result<T, LangError>;
    fn visit_set_array_element(
        &self,
        set_array_element: &SetArrayElementExpr,
    ) -> Result<T, LangError>;
    fn visit_variable(&self, variable: &VariableExpr) -> Result<T, LangError>;
    fn visit_self_ident(&self, self_ident: &SelfIdentExpr) -> Result<T, LangError>;

    fn visit_break(&self) -> Result<T, LangError>;
    fn visit_enum(&self, enum_stmt: &EnumStmt) -> Result<T, LangError>;
    fn visit_impl(&self, impl_stmt: &ImplStmt) -> Result<T, LangError>;
    fn visit_impl_trait(&self, impl_trait: &ImplTraitStmt) -> Result<T, LangError>;
    fn visit_block(&self, block: &BlockStmt) -> Result<T, LangError>;
    fn visit_struct(&self, block: &StructStmt) -> Result<T, LangError>;
    fn visit_expression(&self, block: &ExpressionStmt) -> Result<T, LangError>;
    fn visit_trait(&self, block: &TraitStmt) -> Result<T, LangError>;
    fn visit_trait_function(&self, block: &TraitFunctionStmt) -> Result<T, LangError>;
    fn visit_function(&self, block: &FunctionStmt) -> Result<T, LangError>;
    fn visit_if(&self, block: &IfStmt) -> Result<T, LangError>;
    fn visit_print(&self, block: &PrintStmt) -> Result<T, LangError>;
    fn visit_return(&self, block: &ReturnStmt) -> Result<T, LangError>;
    fn visit_var(&self, block: &VarStmt) -> Result<T, LangError>;
    fn visit_while(&self, block: &WhileStmt) -> Result<T, LangError>;
    fn visit_import(&self, import_stmt: &ImportStmt) -> Result<T, LangError>;
}

pub fn visit_expr<T, V: Visitor<T>>(visitor: &V, expr: &Expr) -> Result<T, LangError> {
    match expr {
        Expr::Assign(ref assign_expr) => Ok(visitor.visit_assign(&*assign_expr)?),
        Expr::Binary(ref binary_expr) => Ok(visitor.visit_binary(&*binary_expr)?),
        Expr::Call(ref call_expr) => Ok(visitor.visit_call(&*call_expr)?),
...
pub fn visit_stmt<T, V: Visitor<T>>(visitor: &V, stmt: &Stmt) -> Result<T, LangError> {
    match stmt {
        Stmt::Break => Ok(visitor.visit_break()?),
        Stmt::Enum(ref enum_stmt) => Ok(visitor.visit_enum(&*enum_stmt)?),
        Stmt::Impl(ref enum_stmt) => Ok(visitor.visit_impl(&*enum_stmt)?),
